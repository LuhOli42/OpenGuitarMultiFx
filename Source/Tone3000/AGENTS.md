...
| `WEBKIT_DISABLE_COMPOSITING_MODE=1` set (Linux only, `Source/Main.cpp`'s `initialise()`, before anything that could spawn the WebKit subprocess) via `setenv` with don't-overwrite | User-reported: the embedded login browser rendered blank. WebKitGTK is documented to render blank under Wayland compositors (this dev machine runs KDE Plasma Wayland) unless its own compositing is disabled — a known category of issue, applied here as the standard mitigation. Read through `OAuthLoginDialog`/`OverlayHost` first — the overlay push/pop and redirect-interception logic look correct, making an environment-level WebKitGTK rendering issue the more likely explanation than a code bug | Assuming it was a code bug, given the review above found none |
| **Follow-up, same day, STILL UNRESOLVED**: reproduced directly (a temporary repro opened `OAuthLoginDialog` against `https://example.com`, not the real TONE3000 URL, isolating the platform issue from anything TONE3000-page-specific) — confirmed `WEBKIT_DISABLE_COMPOSITING_MODE=1` alone does NOT fix the blank render, even for a trivial page. Also tried ADDITIONALLY forcing `GDK_BACKEND=x11` (push the GTK/WebKitGTK widget into XEmbed/X11 embedding instead of native Wayland) — this made it WORSE: the app crashed outright (window created per the log, then gone, no clean shutdown logged). Reverted `GDK_BACKEND=x11` immediately; `WEBKIT_DISABLE_COMPOSITING_MODE=1` stays (harmless even though it didn't fix this machine) | Two standard mitigations for "WebKitGTK blank under Wayland" both failed (one ineffective, one actively harmful) on this specific dev setup (JUCE running inside a `distrobox` container talking to a Wayland host) — this looks like a real, nontrivial platform/embedding issue, not something to keep guessing more env vars at blindly | Continuing to try more env vars speculatively after one already caused a crash -- next real step (not yet done) would be checking JUCE's own GitHub issues for this exact symptom, or testing whether the same code behaves differently outside the distrobox container, rather than more blind guesses |
