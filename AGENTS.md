...
| 5 — Looper, tuner, MIDI, advanced routing | in progress (`LooperProcessor` done -- a single-footswitch-style looper via an edge-detected `Trigger`/`Clear` float-as-button pair, same convention as `HoldProcessor`; tuner + IN/OUT level meters done -- `PitchDetector` (YIN) + peak metering in `AudioEngine`, wired to `FooterBar`'s real gauge/bars; tap-tempo/BPM done -- self-contained wall-clock timing in `FooterBar`, no audio thread involved; MIDI Program Change -> preset switching done; MIDI Learn (CC-to-parameter mapping) done -- doubles as expression pedal support, since an expression pedal is just a continuous CC source (bindings are in-memory only, not yet saved into presets); advanced routing remains) |
